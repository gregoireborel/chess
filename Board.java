...
class Board extends JComponent implements MouseListener {
	...
	for (int i = 0; i < 8; i++)
	    for (int j = 0; j < 8; j++) {
		// If this square contains the selected square, paint it red
		if (this._selectedPiece != NONE && i == this._x && j == this._y)
		    g2d.setColor(Color.RED);
		// Otherwise, alternate between green and yellow
		else if ((i + j) % 2 == 0)
		    g2d.setColor(this._darkGreen);
		else
		    g2d.setColor(this._lightYellow);
		g2d.fillRect(80 * i, 80 * j, 80, 80);
		// If the piece is checking, highlight the square
		if (this.pieceChecking(i, j)) {
		    g2d.setColor(this._hlCheck);
		    g2d.fillRect(80 * i, 80 * j, 80, 80);
		}
		// If this is a valid movement square, highlight the square
		if (this.isMoveValid(i, j)) {
		    g2d.setColor(this._hlValid);
		    g2d.fillRect(80 * i, 80 * j, 80, 80);
		}
	    }
    }
